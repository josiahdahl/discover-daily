...
import { SpotifyTokenDto } from '@discover-daily/integrations/spotify';
import { createSession } from '../services/auth';

function redirectUri(message?: string) {
  return `${process.env.CLIENT_URI}/login${
...
app.get('/api/spotify/callback', async (req, res) => {
  ...
  const validState = await asyncClient(
    'get',
  ...
  try {
    ...
    const authRes = await Axios.post<SpotifyTokenDto>(
